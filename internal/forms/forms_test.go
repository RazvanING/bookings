...
func TestForm_Has(t *testing.T) {
	...
	if !has {
		t.Error("shows form does not have field when is should")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	form.MinLength("x", 10)
	if form.Valid() {
		t.Error("forms shows min length for non existant field")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("x")
	if form.Valid() {
		t.Error("form shows valid email for nonexistent field")
	}
